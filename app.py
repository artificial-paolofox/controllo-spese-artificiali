...
if not df.empty:
    ...
    # === GRAFICI ===
    # --- Grafico a barre (spese per categoria mensili)
    spese = df[df["tipologia"] == "spesa"]

    spese["mese_num"] = spese["data"].dt.strftime("%m")
    spese["periodo"] = spese["mese_num"].map(month_map)
    spese["periodo"] = pd.Categorical(spese["periodo"], categories=month_order, ordered=True)
